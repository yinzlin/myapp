...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let order_with_item1: OrderResponse = client
    ...
    let order_with_item2: OrderResponse = client
        .post(&format!("{}/orders/{}/items", base_url, order_id))
        .json(&item2)
        .send()
        .await?
    ...
    // 7. 设置折扣
    println!("7. 设置 10% 折扣...");
    let discount_req = SetDiscountRequest {
        discount_rate: 0.1,
    };
    let order_with_discount: OrderResponse = client
    ...
    let paid_order: OrderResponse = client
    ...
    let shipped_order: OrderResponse = client
        .post(&format!("{}/orders/{}/status", base_url, order_id))
        .json(&ship_req)
        .send()
        .await?
    ...
    let completed_order: OrderResponse = client
        .post(&format!("{}/orders/{}/status", base_url, order_id))
        .json(&complete_req)
        .send()
        .await?
